...
const StaffProfileDialog = ({
  ...
  useEffect(() => {
    ...
    repository
      .getById(staffId)
      .then(({ data }: { data: Staff }) => setProfile(data))
      .catch(() =>
  ...
  const statusCfg = profile
    ? statusConfig[profile.active ? "active" : "inactive"]
    : null;
        ...
        {!loading && profile && statusCfg && (
          <>
            ...
            <div className="space-y-4 py-2">
              ...
              <InfoRow icon={BadgeCheck} label="Cargo" value={getFirstRole(profile)} />
              {profile.lastLogin && (
                <InfoRow
                  icon={Clock}
                  label="Último acesso"
                  value={new Date(profile.lastLogin).toLocaleString("pt-BR")}
                />
              )}
              {profile.createdAt && (
                <InfoRow
...
const StaffCreateDialog = ({
  ...
  const phoneError = phoneTouched && form.phone.length > 0 && !isValidPhone(form.phone);
  ...
  const handleSubmit = async () => {
    ...
    if (form.phone && !isValidPhone(form.phone)) {
      ...
      toast({ title: "Telefone inválido", description: "Informe um telefone completo com DDD.", variant: "destructive" });
      return;
    }
        ...
        <div className="space-y-4 py-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              {phoneError && (
                <p className="text-xs text-destructive">Telefone incompleto. Ex: (11) 98765-4321</p>
              )}
            </div>
            <div className="space-y-2">
              <Label>Cargo *</Label>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        {fetching ? (
          <div className="space-y-4 py-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2 h-10 rounded-md bg-muted animate-pulse" />
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={loading || fetching}>Cancelar</Button>
...
const StaffPage = () => {
  ...
  const handleView = (item: Staff & BaseUser) => {
