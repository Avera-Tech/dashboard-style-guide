...
import {
  Plus, MoreHorizontal, Eye, Pencil, Trash2, ArrowUpCircle,
} from "lucide-react";
import FilterCard from "@/modules/core/components/financeiro/FilterCard";
import ContaDetailDialog from "@/modules/core/components/financeiro/ContaDetailDialog";
import {
  ContaPagar as ContaPagarType,
  MOCK_CONTAS_PAGAR,
  statusConfig,
  formatCurrency,
  formatDate,
...
const ContasPagar = () => {
  ...
  const categorias = [...new Set(MOCK_CONTAS_PAGAR.map((c) => c.categoria))];

  const filtered = MOCK_CONTAS_PAGAR.filter((c) => {
    const matchName = c.descricao.toLowerCase().includes(filterName.toLowerCase()) || c.fornecedor.toLowerCase().includes(filterName.toLowerCase());
  ...
  return (
    <DashboardLayout>
      ...
      <ContaDetailDialog open={detailOpen} onOpenChange={setDetailOpen} conta={selectedConta} tipo="pagar" />
    </DashboardLayout>
  );
};
